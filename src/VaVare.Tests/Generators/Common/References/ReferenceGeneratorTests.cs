...
namespace VaVare.Tests.Generators.Common.References
{
    ...
    public class ReferenceGeneratorTests
    {
        ...
        {
            Assert.AreEqual("myVariable.MyMethod(1,\"test\")", ReferenceGenerator.Create(new VariableReference("myVariable", new MethodReference("MyMethod", new List<IArgument> { new ValueArgument(1), new StringValueArgument("test")}))).ToString());
        }
